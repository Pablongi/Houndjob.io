...
export const filterJobs = (jobs: Job[], filters: FilterState): Job[] => {
  ...
  if (filters.selectedCategories.size > 0) {
    filteredJobs = filteredJobs.filter(job =>
      job.tags.some(tag => filters.selectedCategories.has(tag.categoría || ''))
    );
  }
  ...
  if (filters.selectedSubcategories.size > 0) {
    filteredJobs = filteredJobs.filter(job =>
      job.tags.some(tag => filters.selectedSubcategories.has(tag.subcategoría || ''))
    );
  }
  ...
  if (filters.selectedTags.size > 0) {
    filteredJobs = filteredJobs.filter(job =>
      job.tags.some(tag => filters.selectedTags.has(tag.tag))
    );
  }
