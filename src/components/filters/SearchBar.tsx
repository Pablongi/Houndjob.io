...
const SearchBar: React.FC<SearchBarProps> = ({ allJobs }) => {
  ...
  const allTerms = useMemo(() => {
    ...
    allJobs.forEach(job => {
      ...
      job.tags.forEach(tag => terms.add(tag.tag));
    });
  ...
  const suggestions = useMemo(() => {
    ...
    let matches = allTerms.filter(term => normalizeText(term).includes(normalizedSearch));
    if (matches.length < 8) {
      const closest = allTerms
        .map(term => ({ term, dist: levenshteinDistance(normalizedSearch, normalizeText(term)) }))
