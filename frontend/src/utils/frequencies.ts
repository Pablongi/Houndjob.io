...
export const computeFrequencies = (jobs: Job[], filters?: FilterState): Frequencies => {
  ...
  jobs.forEach(job => {
    ...
    if (filters) {
      ...
      if (filters.selectedCategories.size > 0 && !job.tags.some((tag: Tag) => filters.selectedCategories.has(tag.categoría || ''))) {
        return;
      }
      if (filters.selectedSubcategories.size > 0 && !job.tags.some((tag: Tag) => filters.selectedSubcategories.has(tag.subcategoría || ''))) {
        return;
      }
      if (filters.selectedTags.size > 0 && !job.tags.some((tag: Tag) => filters.selectedTags.has(tag.tag))) {
        return;
      }
