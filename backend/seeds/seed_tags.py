# /backend/seeds/seed_tags.py
import asyncio
from db import supabase

# =============================================
# TAG_CATEGORIES COMPLETO Y CORREGIDO (Python válido)
# =============================================
TAG_CATEGORIES = [
    {
...
async def seed_tags():
    print("🌱 Iniciando seed SIMPLE y seguro...")

    # Categorías
    for cat in TAG_CATEGORIES:
        supabase.table('categories').insert({"name": cat["categoría"]}).execute()
    print("   ✅ Categorías insertadas")

    # Subcategorías
    for cat in TAG_CATEGORIES:
        cat_row = supabase.table('categories').select('id').eq('name', cat["categoría"]).execute().data[0]
        for sub in cat.get("subcategorías", []):
    ...
    for cat in TAG_CATEGORIES:
        cat_row = supabase.table('categories').select('id').eq('name', cat["categoría"]).execute().data[0]
        for sub in cat.get("subcategorías", []):
            sub_row = supabase.table('subcategories')\
                .select('id')\
                .eq('category_id', cat_row["id"])\
                .eq('name', sub["nombre"]).execute().data[0]
            
            for tag_name in sub.get("tags", []):
